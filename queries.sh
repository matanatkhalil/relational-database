...
echo "$($PSQL "SELECT DISTINCT t.name FROM teams t JOIN games g ON (t.team_id=g.winner_id OR t.team_id=g.opponent_id) WHERE g.round='Eighth-Final' AND g.year=2014 ORDER BY t.name")"
